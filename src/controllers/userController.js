...
const registerController = async (req, res) => {
  try {
    ...
    const transporter = nodemailer.createTransport({
      service: "Gmail",
      auth: {
        user: "irshadahmadmir2017@gmail.com",
        pass: "setkhbmkuorlmyla",
      },
    });

    const options = {
      from: "irshadahmadmir2017@gmail.com",
      to: email,
      subject: "Login OTP",
...
const loginController = async (req, res) => {
  try {
    ...
    if (findUser.blockedUntil && findUser.blockedUntil > Date.now()) {
      const remainingTime = Math.ceil((findUser.blockedUntil - Date.now()) / 1000 / 60);
      return res.status(403).json({
        success: false,
        message: "Account blocked. Please try again after " + remainingTime + " minutes",
      });
    }
    ...
    const otpExpirationTime = findUser.otpGenerationTime.getTime() + 5 * 60 * 1000; // OTP is valid for 5 minutes
